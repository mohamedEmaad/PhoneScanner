...
//        (?:\+-?)?                # Potential international prefix, may have -
//        [(]?                    # Potential opening (
//        \b(\w{3})                # Capture xxx
//        [)]?                    # Potential closing )
//        [\ -./]?                # Potential separator
...
"""#
        ...
        print("here is the text \(self)")

        // Potential number found. Strip out punctuation, whitespace and country
        // prefix.
        var phoneNumberDigits = ""
        let substring = String(self[range])
        print("here is the range \(substring)")
        let nsrange = NSRange(substring.startIndex..., in: substring)
        print("here is the ns range \(nsrange)")
        do {
            // Extract the characters from the substring.
            let regex = try NSRegularExpression(pattern: pattern2, options: [])
            phoneNumberDigits = regex.stringByReplacingMatches(in: substring, options: [], range: nsrange, withTemplate: "")
        } catch {
            print("Error \(error) when creating pattern")
        }

        // Must be exactly 10 digits.
        guard 8...15 ~= phoneNumberDigits.count else {
            return nil
        }
